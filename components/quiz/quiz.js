...
window.quizModule = (() => {
  ...
  function initQuiz(quizQuestions, answers, questionIndex, score) {
    console.log("quizModule.initQuiz called with", quizQuestions.length, "questions");
  ...
  function displayCurrentQuestion() {
    console.log("quizModule.displayCurrentQuestion called, currentQuestionIndex:", currentQuestionIndex);
    ...
    if (currentQuestion.type === "multiple-choice") {
      displayMultipleChoiceQuestion(container, currentQuestion, userAnswer, showFeedback);
    } else if (currentQuestion.type === "drag-and-drop") {
      displayDragAndDropQuestion(container, currentQuestion, userAnswer, showFeedback);
    } else if (currentQuestion.type === "match-answer") {
      displayMatchAnswerQuestion(container, currentQuestion, userAnswer, showFeedback);
    } else {
      // Fallback to multiple choice display if type is not specified
      console.warn("Unknown question type, falling back to multiple-choice:", currentQuestion.type);
      displayMultipleChoiceQuestion(container, currentQuestion, userAnswer, showFeedback);
    }
  }

  // Display multiple choice question
  function displayMultipleChoiceQuestion(container, currentQuestion, userAnswer, showFeedback) {
    let questionHtml = `
          <div class="question-block">
        <h3 class="text-xl font-bold mb-4">${currentQuestionIndex + 1}. ${currentQuestion.question}</h3>
    ...
    currentQuestion.choices.forEach((choice, index) => {
      ...
      let optionClass = "answer-option p-3 border rounded-lg cursor-pointer hover:bg-blue-50 transition";
      let feedbackIcon = "";

      if (showFeedback) {
        if (isSelected) {
          optionClass += isCorrect ? " correct bg-green-100 border-green-500" : " incorrect bg-red-100 border-red-500";
          ...
          feedbackIcon = '<span class="feedback-icon text-green-600 ml-2">✓</span>';
        }
      } else if (isSelected) {
  ...
  function displayDragAndDropQuestion(container, currentQuestion, userAnswer, showFeedback) {
      ...
      if (userAnswer && userAnswer.length) {
        ...
        for (let i = 0; i < userAnswer.length; i++) {
          ...
          if (itemIndex >= 0 && itemIndex < items.length) {
            ...
            const itemClass = isCorrect ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50";
  ...
  function displayMatchAnswerQuestion(container, currentQuestion, userAnswer, showFeedback) {
    ...
    if (showFeedback) {
      ...
      currentQuestion.matches.forEach(match => {
        const leftItem = columnA.find(item => item.startsWith(match.left + "."));
        const rightItem = columnB.find(item => item.startsWith(match.right + "."));
      ...
      if (userAnswer && userAnswer.length) {
        ...
        userAnswer.forEach(userMatch => {
          const correctMatch = currentQuestion.matches.find(m => m.left === userMatch.left);
          const isCorrect = correctMatch && correctMatch.right === userMatch.right;
          
          const leftItem = columnA.find(item => item.startsWith(userMatch.left + "."));
          const rightItem = columnB.find(item => item.startsWith(userMatch.right + "."));
      ...
      questionHtml += `
        ...
        columnB.forEach((item, index) => {
          ...
          questionHtml += `
            <li class="match-item-b p-3 bg-white rounded border border-blue-300 flex items-center cursor-pointer" 
  ...
  function initDragAndDrop() {
    ...
    submitBtn.addEventListener('click', function() {
      ...
      const userSequence = orderedItems.map(item => parseInt(item.dataset.index));
  ...
  function initMatchAnswerEvents() {
    ...
    function updateConnectionsDisplay() {
      ...
      if (connections.length === 0) {
        ...
        connections.forEach((conn, index) => {
          ...
          li.className = 'p-2 bg-white rounded border border-blue-200 flex justify-between items-center';
          
          // Find the full text of the items
          const itemAText = Array.from(itemsA).find(el => el.dataset.item === conn.left)?.textContent.trim() || '';
          const itemBText = Array.from(itemsB).find(el => el.dataset.item === conn.right)?.textContent.trim() || '';
    ...
    function drawConnectionLines() {
      ...
      connections.forEach(conn => {
        const itemA = Array.from(itemsA).find(el => el.dataset.item === conn.left);
        const itemB = Array.from(itemsB).find(el => el.dataset.item === conn.right);
    ...
    itemsB.forEach(item => {
      item.addEventListener('click', function() {
        ...
        const existingConnection = connections.findIndex(c => c.left === selectedItemA);
    ...
    submitBtn.addEventListener('click', function() {
      ...
      if (connections.length !== currentQuestion.columnA.length) {
        alert(`Please connect all ${currentQuestion.columnA.length} items from Column A before submitting.`);
        return;
      }
  ...
  async function selectDragAndDropAnswer(userSequence) {
    ...
    // Check if answer is correct
    const currentQuestion = questions[currentQuestionIndex];
    const correctSequence = currentQuestion.correctSequence.map(num => num - 1); // Convert to 0-based
  ...
  function changeBackgroundColor(isCorrect) {
    ...
    if (quizSection) {
      quizSection.classList.remove("correct-background", "incorrect-background");
    }
